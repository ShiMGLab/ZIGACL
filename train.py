from sklearn.cluster import KMeans
import torch
import torch.optim.lr_scheduler as lr_scheduler
import utils
from loss import ZINBLoss
import numpy as np
from sklearn.metrics import silhouette_score, adjusted_rand_score, normalized_mutual_info_score, davies_bouldin_score
import time
import random


import torch.nn.functional as F

random.seed(1)

def train(M, init_model, Zscore_data, rawData, adj, size_factor, device, args):
    start_time = time.time()

    start_mem = torch.cuda.max_memory_allocated(device=device)

    init_model.to(device)
    data = torch.Tensor(Zscore_data).to(device)
    sf = torch.autograd.Variable((torch.from_numpy(size_factor[:,None]).type(torch.FloatTensor)).to(device),
                           requires_grad=True)
    optimizer = torch.optim.Adam(init_model.parameters(), lr=args.lr)
    # adj = torch.Tensor(adj).to(device)
    # adj = torch.tensor(adj, dtype=torch.long).to(device)
    scheduler = lr_scheduler.StepLR(optimizer, step_size=40, gamma=0.5, last_epoch=-1)
    best_model = init_model
    loss_update = 100000
    for epoch in range(args.training_epoch):
        z, pi, mean, disp, pred, q, ae_p = init_model(data, adj, M)
        l = ZINBLoss(theta_shape=(args.n_input,))
        zinb_loss = l(mean * sf, pi, target=torch.tensor(rawData).to(device), theta=disp)
        ce_loss = F.kl_div(pred.log(), ae_p, reduction='batchmean')

        loss = zinb_loss + 0.1*ce_loss

        if (epoch+1) % 10   == 0:
            print("epoch %d, loss %.4f, zinb_loss %.4f, dgi_loss %.4f"
                            % (epoch+1, loss, zinb_loss, ce_loss))

        if loss_update > loss:
            loss_update = loss
            best_model = init_model
            epoch_update = epoch

        if ((epoch - epoch_update) > 50):
            print("Early stopping at epoch {}".format(epoch_update))
            elapsed_time = time.time() - start_time
            max_mem = torch.cuda.max_memory_allocated(device=device) - start_mem
            print("Finish Training! Elapsed time: {:.4f} seconds, Max memory usage: {:.4f} MB".format(elapsed_time, max_mem / 1024 / 1024))
            return best_model
        optimizer.zero_grad()
        loss.backward()
        torch.nn.utils.clip_grad_norm_(init_model.parameters(), max_norm=3, norm_type=2)
        optimizer.step()
        scheduler.step()
    elapsed_time = time.time() - start_time
    max_mem = torch.cuda.max_memory_allocated(device=device) - start_mem
    print("Finish Training! Elapsed time: {:.4f} seconds, Max memory usage: {:.4f} MB".format(elapsed_time, max_mem / 1024 / 1024))
    return best_model, elapsed_time

alpha = 1
def loss_func(z, cluster_layer):
    q = 1.0 / (1.0 + torch.sum((z.unsqueeze(1) - cluster_layer) ** 2, dim=2) / alpha)
    q = q ** (alpha + 1.0) / 2.0
    q = (q.t() / torch.sum(q, dim=1)).t()

    weight = q ** 2 / torch.sum(q, dim=0)
    p = (weight.t() / torch.sum(weight, dim=1)).t()

    log_q = torch.log(q)
    loss = torch.nn.functional.kl_div(log_q, p, reduction='batchmean')
    return loss, p

def clustering(M, pretrain_model, Zscore_data, rawData, celltype, adj, size_factor, device, args):
    start_time = time.time()
    print(rawData.shape)

    start_mem = torch.cuda.max_memory_allocated(device=device)

    data = torch.Tensor(Zscore_data).to(device)
    model = pretrain_model.to(device)
    sf = torch.autograd.Variable((torch.from_numpy(size_factor[:,None]).type(torch.FloatTensor)).to(device),
                          requires_grad=True)
    #cluster center
    with torch.no_grad():
        z, _, _, _, _, _, _ = model(data, adj, M)

    kmeans = KMeans(n_clusters=args.n_clusters, n_init=args.n_init)
    y_pred = kmeans.fit_predict(z.data.cpu().numpy())
    y_pred_last = y_pred
    model.cluster_layer.data = torch.tensor(kmeans.cluster_centers_).to(device)

    optimizer = torch.optim.Adam(list(model.ae.enc_1.parameters()) + list(model.ae.enc_2.parameters()) +
                                  list(model.gat_model.parameters())  +
                                 list(model.ae.z_layer.parameters()), lr=0.001)


    for epoch in range(args.clustering_epoch):
        if epoch % 1 == 0:
            _, _, _, _, pred, q, ae_p = model(data, adj, M)
            log_q = torch.log(q)
            kl_loss = torch.nn.functional.kl_div(log_q, ae_p, reduction='batchmean')

        z, pi, mean, disp, pred, q, _ = model(data, adj, M)
        l = ZINBLoss(theta_shape=(args.n_input,))
        zinb_loss = l(mean * sf, pi, target=torch.tensor(rawData).to(device), theta=disp)

        print(pred.size())
        print(ae_p.size())
        ce_loss = F.kl_div(pred.log(), ae_p, reduction='batchmean')
        loss = 0.1*zinb_loss + kl_loss + 0.01*ce_loss
        loss = zinb_loss + 0.1*kl_loss + 0.01*ce_loss

        loss.requires_grad_(True)
        label = utils.dist_2_label(ae_p)



        asw = silhouette_score(z.detach().cpu().numpy(), label)
        print("asw?", asw)
        db = davies_bouldin_score(z.detach().cpu().numpy(), label)
        # ari = adjusted_rand_score(celltype, label)
        # nmi = normalized_mutual_info_score(celltype, label)


        if (epoch+1) % 10 == 0:
            print("epoch %d, loss %.4f, kl_loss %.4f, ASW %.3f"% (epoch+1, loss, kl_loss, asw))
        num = data.shape[0]
        tol=1e-3
        if epoch == 0:
            last_label = label
        else:
            delta_label = np.sum(label != last_label).astype(np.float32) / num
            last_label = label
            if epoch>20 and delta_label < tol:
                print('delta_label ', delta_label, '< tol ', tol)
                print("Reach tolerance threshold. Stopping training.")
                elapsed_time = time.time() - start_time
                max_mem = torch.cuda.max_memory_allocated(device=device) - start_mem
                print("Elapsed time: {:.4f} seconds, Max memory usage: {:.4f} MB".format(elapsed_time, max_mem / 1024 / 1024))
                break
        optimizer.zero_grad()
        loss.backward()
        torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=3, norm_type=2)
        optimizer.step()
    elapsed_time = time.time() - start_time
    max_mem = torch.cuda.max_memory_allocated(device=device) - start_mem
    print("Finish Clustering! Elapsed time: {:.4f} seconds, Max memory usage: {:.4f} MB".format(elapsed_time, max_mem / 1024 / 1024))
    umap_save_path = ['./umap_figure/%s_pred_label.png' % (args.name), './umap_figure/%s_true_label.png' % (args.name)]


    print('我改的')
    asw = np.round(silhouette_score(z.detach().cpu().numpy(), last_label), 3)
    db = np.round(davies_bouldin_score(z.detach().cpu().numpy(), last_label), 3)
    nmi = np.round(normalized_mutual_info_score(celltype, last_label), 3)
    ari = np.round(adjusted_rand_score(celltype, last_label), 3)
    print("Final ASW %.3f, DB %.3f, ARI %.3f, NMI %.3f" % (asw, db, ari, nmi))

    utils.umap_visual(z.detach().cpu().numpy(),
                      label=last_label,
                      title='predicted label',
                      save_path=umap_save_path[0],
                      asw_used=None)
    utils.umap_visual(z.detach().cpu().numpy(),
                      label=celltype,
                      title='true label',
                      save_path=umap_save_path[1])
    asw = np.round(silhouette_score(z.detach().cpu().numpy(), last_label), 3)
    db = np.round(davies_bouldin_score(z.detach().cpu().numpy(), last_label), 3)
    nmi = np.round(normalized_mutual_info_score(celltype, last_label), 3)
    ari = np.round(adjusted_rand_score(celltype, last_label), 3)
    print("Final ASW %.3f, DB %.3f, ARI %.3f, NMI %.3f" % (asw, db, ari, nmi))
    print("結束")

    return [asw,db], last_label, model, elapsed_time

